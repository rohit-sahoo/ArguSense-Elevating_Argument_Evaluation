...
st.markdown("<h1><em>ArguSense: Intelligent NLP-driven Argument Evaluation and Effectiveness Analysis</em></h1>",
...
if st.button("Proceed"):
    # Display the tf-lonformer-v12
    st.write(user_input)

## model building start


tokenizer = AutoTokenizer.from_pretrained('tf-lonformer-v12')
...
model.load_weights('long_v14.h5')
...
def tok_func(x): return loaded_tokz(x["inputs"], truncation=True)
...
def get_dds(df, train=True):
    ...
    to_remove = ['discourse_text', 'discourse_type', 'inputs', 'discourse_id', 'id', 'predictionstring',
...
loaded_preds = F.softmax(torch.Tensor(loaded_trainer.predict(loaded_test_ds).predictions)).numpy().astype(float)
